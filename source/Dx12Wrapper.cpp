...
{
    com_ptr<ID3D12Debug> debugLayer = nullptr;
...
Dx12Wrapper::GetTextureByPath(const char* texpath)
{
    ...
    if(it != _textureTable.end()) {
        ...
        return com_ptr<ID3D12Resource>(CreateTextureFromFile(texpath));
    }
}
...
{
    _loadLambdaTable["sph"] = _loadLambdaTable["spa"] = _loadLambdaTable["bmp"] = _loadLambdaTable["png"] = _loadLambdaTable["jpg"] = [](const std::wstring& path, DirectX::TexMetadata* meta, DirectX::ScratchImage& img) -> HRESULT {
        return DirectX::LoadFromWICFile(path.c_str(), 0, meta, img);
    };

    _loadLambdaTable["tga"] = [](const std::wstring& path, DirectX::TexMetadata* meta, DirectX::ScratchImage& img) -> HRESULT {
        return DirectX::LoadFromTGAFile(path.c_str(), meta, img);
    };

    _loadLambdaTable["dds"] = [](const std::wstring& path, DirectX::TexMetadata* meta, DirectX::ScratchImage& img) -> HRESULT {
