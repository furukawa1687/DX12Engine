...
// LookAt(指定した座標を向く)
//---------------------------------------------------------------------------
void PMDActor::LookAt(f32 x, f32 y, f32 z)
{
...
// ベジェ曲線の取得
//---------------------------------------------------------------------------
float PMDActor::GetYFromXOnBezier(f32 x, const XMFLOAT2& a, const XMFLOAT2& b, u8 n)
{
    if(a.x == a.y && b.x == b.y)
        return x;   //計算不要
    float       t  = x;
...
// IK解決
//---------------------------------------------------------------------------
void PMDActor::IKSolve(s32 frameNo)
{
    //いつもの逆から検索
...
#pragma pack()
    ...
    auto getNameFromIdx = [&](uint16_t idx) -> std::string {
        auto it = find_if(_boneNodeTable.begin(), _boneNodeTable.end(), [idx](const std::pair<std::string, BoneNode>& obj) {
